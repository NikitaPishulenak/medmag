...
{
   $id=substr($id,0,32);
   $result=mysql_query("SELECT id,file FROM iws_arfiles_records WHERE pse='".$id."'");
   if(mysql_numrows($result)>=1){
      list($idFile,$fileTo)=mysql_fetch_row($result);
      if(is_file($path."/FilesForDownload/".$fileTo)){
         $arrRet["filesize"]=filesize($path."/FilesForDownload/".$fileTo);
         $arrRet["filetime"]=date("D, d M Y H:i:s T", filemtime($path."/FilesForDownload/".$fileTo));
         $arrRet["filepath"]=$path."/FilesForDownload/".$fileTo;
      ...
      if ( preg_match("/Mozilla|Opera/i", $useragent) && !preg_match("!Bot/|robot|Slurp/|yahoo!i", $useragent))
      {
               ...
               mysql_query("INSERT INTO iws_statistics (dt,frm,ip_adr,url,menu,lng,coockie) VALUES ('".date('Y-m-d H:i:s', time())."','".$hostT['host']."','".$tempar[1]."','files',$idFile,'ru','".$tempar[0]."')");
      }
