...
mysql_query('SET NAMES "cp1251"');
    ...
    if(isset($act) && $act == "prefOk"){
         ...
         $sql="UPDATE iws_arfiles_prefernce SET usertrue=".(($usertrue) ? '1' : '0').",countinmain=$countinmain,countinpage=$countinpage,space=$space,extention='$extention' WHERE id=1";
         if(!mysql_query($sql)){
            header("location: pref.php?err=1");
            return;
         } else {    
            echo "<HTML><HEAD>
               <title>Изменение настроек публикаций</title>
               </head><body bgcolor=buttonface>
               <h4>Настройки модуля публикаций успешно сохранены</h4>
               <script><!--
         ...
         list($usertrue,$countinmain,$countinpage,$space,$extention)=mysql_fetch_row(mysql_query("SELECT usertrue,countinmain,countinpage,space,extention FROM iws_arfiles_prefernce WHERE id=1"));

         echo "<table width=100% border=0 cellpadding=3 cellspacing=0>
            <tr><td align=center class=usr>Настройки публикаций</td></tr></table>
            <script><!--
            function submitr(fr)
            {
