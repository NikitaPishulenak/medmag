...
{
    global $docRoot;
    $file_size = filesize($docRoot."/FilesForDownload_C/".$file);
...
{
   global $docRoot;
   if(is_file($docRoot."/FilesForDownload_C/".$fileToDel)) unlink($docRoot."/FilesForDownload_C/".$fileToDel);
}
...
//    if($_FILES['filearchive']['error'] == 1 || $_FILES['filearchive']['size'] > ($mb*1048576) || $_FILES['filearchive']['size']; == 0) return false;
      ...
      if(!is_dir($docRoot."/FilesForDownload_C")) mkdir($docRoot."/FilesForDownload_C", 0755);

      $file = encodestring(strtolower(str_replace(" ", "", basename($_FILES['filearchive']['name']))));
      $file = date("YmdHis").$file;

      move_uploaded_file($_FILES['filearchive']['tmp_name'], $docRoot."/FilesForDownload_C/".$file);  return $file;
