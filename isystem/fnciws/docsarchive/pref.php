...
mysql_query('SET NAMES "cp1251"');
    ...
    if(isset($act) && $act == "prefOk"){
         ...
         $sql="UPDATE iws_ardoc_prefernce SET usertrue=".(($usertrue) ? '1' : '0').",countinmain=$countinmain,countinpage=$countinpage,space=$space,extention='$extention' WHERE id=1";
         if(!mysql_query($sql)){
            header("location: pref.php?err=1");
            return;
         } else {    
            echo "<HTML><HEAD>
               <title>Изменение настроек архива документов</title>
               </head><body bgcolor=buttonface>
               <h4>Настройки модуля архива документов успешно сохранены</h4>
               <script><!--
         ...
         list($usertrue,$countinmain,$countinpage,$space,$extention)=mysql_fetch_row(mysql_query("SELECT usertrue,countinmain,countinpage,space,extention FROM iws_ardoc_prefernce WHERE id=1"));

         echo "<table width=100% border=0 cellpadding=3 cellspacing=0>
            <tr><td align=center class=usr>Настройки архива документов</td></tr></table>
            <script><!--
            function submitr(fr)
            {
            ...
            <tr><td align=right>Количество документов на первой странице сайта</td><td><input name=countinmain size=4 maxlength=2 value=\"".$countinmain."\"> Не менее одного</td></tr>
            <tr><td align=right>Количество документов на одной странице в архиве</td><td><input name=countinpage size=4 maxlength=3 value=\"".$countinpage."\"> Не менее одного</td></tr>
            <tr><td colspan=2><hr></td></tr>
            <tr valign=top><td align=right>Разрешить добавлять документы пользователям<br><br></td><td>
            <input class=chk type=checkbox name=\"usertrue\"";

            if($usertrue){ echo " checked"; }

            echo "></td></tr>
            <tr valign=top><td align=right>Максимальный объем документова на закачку</td><td><input name=space size=4 maxlength=2 value=\"".$space."\"> Mb<br>Если установите 0Mb, то ограничений не будет</td></tr>
